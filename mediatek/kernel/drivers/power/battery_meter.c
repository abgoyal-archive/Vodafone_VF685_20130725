...
void oam_init(void)
{
	...
	kal_int32 sw_vol = 0;
	kal_int32 sw_current = 0;
	kal_int32 sw_r = 0;
	kal_int32 sw_soc = 0;
	char *p = NULL;
	p = strstr(saved_command_line, "SW_VOL=");
	if(p != NULL)
	{
		p += 7;
		sw_vol = simple_strtol(p, NULL, 10);
		printk("CHENFEI SW_VOL %d\n",sw_vol);
	}
	else
	{
		printk("CHENFEI saved_command_line get error \n");
	}
	p = strstr(saved_command_line, "SW_CUR=");
	if(p != NULL)
	{
		p += 7;
		sw_current = simple_strtol(p, NULL, 10);
		printk("CHENFEI SW_CUR %d \n",sw_current);
	}
	else
	{
		printk("CHENFEI saved_command_line get error \n");
	...
	{
		sw_r = fgauge_read_r_bat_by_v(sw_vol);
		sw_vol = sw_vol - (sw_current*sw_r/1000);
		sw_soc = fgauge_read_capacity_by_v(sw_vol);
		printk("CHENFEI SW_VOL = %d, SW_SOC = %d , HW_VOL = %d , HW_SOC = %d \n",sw_vol,sw_soc,gFG_voltage,gFG_capacity_by_v);
		if((sw_soc < gFG_capacity_by_v) && ((gFG_capacity_by_v - sw_soc) >= 5))
		{
			gFG_voltage = sw_vol;
			gFG_capacity_by_v = sw_soc;
		}
	}
...
void oam_run(void)
{
	...
	if(gFG_Is_Charging == KAL_FALSE)
	{
		...
		{
			if(oam_i_1 > 6000 && oam_i_2 > 6000)
			{
				oam_i_1 = oam_i_1 + oam_i_1 - 6000;
				oam_i_2 = oam_i_2 + oam_i_2 - 6000;
			}
		}
		else if(BMT_status.SOC > 60)
		{
			if(oam_i_1 > 5000 && oam_i_2 > 5000)
			{
				oam_i_1 = oam_i_1 + oam_i_1 - 5000;
				oam_i_2 = oam_i_2 + oam_i_2 - 5000;
			}
		}
		else if(BMT_status.SOC > 40)
		{
			if(oam_i_1 > 4000 && oam_i_2 > 4000)
			{
				oam_i_1 = oam_i_1 + oam_i_1 - 4000;
				oam_i_2 = oam_i_2 + oam_i_2 - 4000;
			}
		}
		else if(BMT_status.SOC > 20)
		{
			if(oam_i_1 > 3000 && oam_i_2 > 3000)
			{
				oam_i_1 = oam_i_1 + oam_i_1 - 3000;
				oam_i_2 = oam_i_2 + oam_i_2 - 3000;
			}
		}
		else if(BMT_status.SOC > 0)
		{
			if(oam_i_1 > 2000 && oam_i_2 > 2000)
			{
				oam_i_1 = oam_i_1 + oam_i_1 - 2000;
				oam_i_2 = oam_i_2 + oam_i_2 - 2000;
			}
		}
	}	
	#endif
